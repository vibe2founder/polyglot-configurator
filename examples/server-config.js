...
async function demonstrateServerConfig() {
  ...
  console.log(`🚀 Iniciando servidor ${global.getConfig("app.name")} v${global.getConfig("app.version")}...`);
  ...
  if (loggingConfig.transports.includes("console")) {
    console.log(`   📝 Logging ativado - Nível: ${loggingConfig.level}, Formato: ${loggingConfig.format}`);
  }
  console.log(`   ⏱️  Timeout configurado: ${apiConfig.timeout}ms`);
  console.log(`   🛡️  Rate Limiting: ${apiConfig.rateLimit.max} requests por ${apiConfig.rateLimit.windowMs / 1000}s`);
  ...
  console.log(`✅ Servidor iniciado com sucesso em http://${serverConfig.host}:${serverConfig.port}`);
  ...
  return new Promise((resolve) => {
    setTimeout(() => {
      const responses = {
        ...
        "/api/data": { data: "Sample data from API", timestamp: new Date().toISOString() }
      };
