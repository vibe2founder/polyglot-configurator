...
console.log(
  `   NODE_ENV: ${process.env.NODE_ENV} (${typeof process.env.NODE_ENV})`
);
console.log(`   PORT: ${process.env.PORT} (${typeof process.env.PORT})`);
console.log(`   DEBUG: ${process.env.DEBUG} (${typeof process.env.DEBUG})`);
console.log(
  `   API_URL: ${process.env.API_URL} (${typeof process.env.API_URL})`
);
console.log(
  `   DATABASE_URL: ${process.env.DATABASE_URL} (${typeof process.env
    .DATABASE_URL})`
);
console.log(
  `   JWT_SECRET: ${process.env.JWT_SECRET?.substring(
    0,
    10
...
console.log(`   Rate Limit Max: ${global.CONFIG?.json?.api?.rateLimit?.max}`);
console.log(
  `   Rate Limit Window: ${global.CONFIG?.json?.api?.rateLimit?.windowMs}`
);

console.log("\n🔄 Configuração mesclada (all):");
console.log(`   App Name: ${global.CONFIG?.all?.app?.name}`);
console.log(
  `   App Port: ${global.CONFIG?.all?.app?.port} (${typeof global.CONFIG?.all
    ?.app?.port})`
);
console.log(
  `   App Debug: ${global.CONFIG?.all?.app?.debug} (${typeof global.CONFIG?.all
    ?.app?.debug})`
);
console.log(`   Database Host: ${global.CONFIG?.all?.database?.host}`);
console.log(`   API Base URL: ${global.CONFIG?.all?.api?.baseUrl}`);
console.log(
  `   JWT Secret: ${global.CONFIG?.all?.auth?.jwt?.secret?.substring(0, 10)}...`
);
...
console.log(
  `   getConfig('database.host'): ${global.getConfig("database.host")}`
);
console.log(`   getConfig('api.timeout'): ${global.getConfig("api.timeout")}`);
console.log(
  `   getConfig('auth.jwt.secret'): ${global
    .getConfig("auth.jwt.secret")
    ?.substring(0, 10)}...`
);
